...
## 当前对齐进度（真实扫描，177 case v_* 变体）

| 胶片类型 | 总数 | STRICT | PASS | WARN | FAIL |
|---|---|---|---|---|---|
| POS | 59 | 4 | 53 | 0 | 2 |
| NEG | 59 | 2 | 53 | 2 | 1 |
| BW | 59 | 0 | 0 | 1 | **58** |
...
色卡硬边 / tile 黑边 transition 放大 USM 实现微差，WARN 多；真实扫描无硬边，同功能 case 在真实场景下普遍 PASS+。
...
## v6 阶段：T25 ColorCorrection apply chroma permutation 修复（2026-04-20）

Agent 字节级 decompile `FUN_702d4b50` R-row，发现 Rust 实现 chroma 数组顺序错 +
M3x6 列与 chroma 之间缺 `PERM=[2,1,0,5,4,3]` permutation。

### v6 基线快照（启用 ApplyCC apply）

TOTAL: **7 STRICT / 4 PASS / 4 WARN / 130 FAIL** (145 tests)

对比 v5: 6/0/5/134 → +1 STRICT / +4 PASS / -1 WARN / -4 FAIL

关键 T6 改进：
| case | v5 baseline | v6 after fix | status |
|------|-------------|--------------|--------|
| ext_rgb_saturated   | 865 WARN  | 865 (some tests → STRICT) | 1 STRICT + 2 WARN + 2 FAIL |
| emb_neg_rgb_standard | 931 FAIL | **276 PASS** | PASS tier ✓ |
| emb_neg_rgb_saturated | 1513 FAIL | **304 PASS** | PASS tier ✓ |
| ext_neg_rgb_standard  | 931 FAIL | **276 PASS** | PASS tier ✓ |
| ext_neg_rgb_saturated | 1513 FAIL | **304 PASS** | PASS tier ✓ |

### v6 残余回归

- `e2e_all_config` T6: 3798 → 7203（+3405 vs v5，但比 v5 wrong perm 的 8780 好）
